defmodule Ecto.Adapters.Exqlite.Connection do
  ...
  def stream(conn, sql, params, opts) do
    query = %Exqlite.Query{statement: sql}
    %Exqlite.Stream{conn: conn, query: sql, params: params, options: opts}
  end
  ...
  def insert(prefix, table, header, rows, on_conflict, returning) do
    insert(prefix, table, header, rows, on_conflict, [], [])
  end
  ...
  def insert(_prefix, _table, _header, _rows, _on_conflict, _returning, _placeholders) do
    raise ArgumentError, ":placeholders is not supported by SQLite3"
  end
  ...
  defp insert_all_value({%Ecto.Query{} = query, _params_counter}),
    do: [?(, all(query), ?)]
  ...
  defp join_qual(mode, q),
    do: error!(q, "join `#{inspect(mode)}` not supported by SQLite3")
  ...
  # Sqlite has some json operation support
  #
  ...
  defp column_change(_table, {_command, _name, %Reference{validate: false}, _opts}) do
    error!(nil, "validate: false on references is not supported in Exqlite")
  end
  ...
  defp after_expr(column) when is_atom(column) or is_binary(column),
    do: " AFTER `#{column}`"
  ...
  defp options_expr(keyword) when is_list(keyword),
    do: error!(nil, "SQLite3 adapter does not support keyword lists in :options")
  ...
  defp quote_entity(val) when is_atom(val), do: quote_entity(Atom.to_string(val))
